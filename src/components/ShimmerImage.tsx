...
export const ShimmerImage: React.FC<ShimmerImageProps> = ({
  ...
  return (
    <div className={`relative overflow-hidden ${containerClassName}`}>
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-[#151515] z-10 overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-30">
            <div className="w-6 h-6 border-2 border-white/20 border-t-[#C5FF4A] rounded-full animate-spin"></div>
          </div>
        </div>
      )}
      ...
      {isError && (
        <div className="absolute inset-0 bg-[#181818] flex flex-col items-center justify-center text-neutral-500 text-xs p-4 text-center">
          <span className="font-serif-heading text-neutral-400 font-bold mb-1">Safar-E-Jahan</span>
          <span className="text-[10px] font-mono text-neutral-500">Holy Sanctuary Archive</span>
        </div>
      )}
    </div>
  );
